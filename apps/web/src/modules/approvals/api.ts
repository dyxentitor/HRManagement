...
export async function approveItem(
  ...
  if (kind === "leave") {
    ...
    const resp = await authFetch(`${BASE_URL}/api/v1/schedule/swap-requests/${id}/approve/`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function rejectItem(
  ...
  if (kind === "leave") {
    ...
    const resp = await authFetch(`${BASE_URL}/api/v1/schedule/swap-requests/${id}/reject/`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ note: comment }),
    })
