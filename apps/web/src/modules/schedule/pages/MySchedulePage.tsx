...
import { type CalendarHoliday, type Shift, type ShiftAssignment, scheduleApi } from "../api"
import { HolidayCard } from "../components/HolidayCard"
import { MySwapRequests } from "../components/MySwapRequests"
import { type DayShift, ScheduleDayCard } from "../components/ScheduleDayCard"
...
export default function MySchedulePage() {
  ...
  const [weekStart, setWeekStart] = useState<string>(() => startOfWeekIsoLocal(new Date()))
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setHolidays(lists.flat().map((h) => ({ date: h.date, name: h.name, type: h.type })))
    } catch {
      setHolidays([])
    }
  }, [weekStart, weekEnd])
  ...
  const dayModels = days.map((iso) => {
    ...
    return {
      ...
      swapAssignmentId: a?.is_published && a.status === "scheduled" && iso > todayIso ? a.id : null,
    }
  })
  ...
  const totalHours = Math.round(
    dayModels.reduce((sum, d) => {
      ...
      return sum + (sh ? shiftHours(sh.start_time, sh.end_time, sh.crosses_midnight) : 0)
    }, 0),
  )
      ...
      <section className="bg-surface-hover border border-border-subtle rounded-lg p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
          {dayModels.map((d) => (
            <ScheduleDayCard
              ...
              shift={d.shift}
              onRequestSwap={d.swapAssignmentId ? () => setSwapFor(d.swapAssignmentId) : undefined}
            />
          ))}
        </div>
        ...
        {swapFor && (
          <SwapRequestDrawer
            ...
            myShiftLabel={dayModels.find((d) => d.swapAssignmentId === swapFor)?.shift?.name ?? ""}
        ...
        <div className="mt-4 border-t border-border-subtle pt-4">
          <h3 className="text-label uppercase text-text-tertiary mb-2">Holidays in {monthLabel}</h3>
          {monthHolidays.length > 0 ? (
            ...
            <p className="text-small text-text-tertiary">No public holidays in {monthLabel}.</p>
          )}
        </div>
      </section>
