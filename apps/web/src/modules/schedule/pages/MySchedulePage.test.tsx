...
const holidayDate =
  addDaysIso(weekStart, 2) === todayIsoLocal() ? addDaysIso(weekStart, 3) : addDaysIso(weekStart, 2)
...
describe("MySchedulePage holidays", () => {
  ...
  it("shows an empty state when the visible month has no holidays", async () => {
    ...
    renderPage()
    expect(await screen.findByText(/No public holidays in/)).toBeInTheDocument()
  })
})
...
describe("MySchedulePage swap action", () => {
  it("shows Request swap button for a future published scheduled assignment", async () => {
    ...
    expect(await screen.findByRole("button", { name: /request swap/i })).toBeInTheDocument()
  })
