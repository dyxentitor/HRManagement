  ...
  return (
    <div
      className={cn(
        ...
        isToday ? "border-accent-500 ring-1 ring-accent-500/40" : "border-border-subtle",
        isWeekend && !holidayName && !isToday && "bg-surface-elevated",
      )}
    >
      <div className="flex items-center justify-between">
        <span
          className={cn("text-label uppercase", holidayName ? "text-peach" : "text-text-tertiary")}
        >
      ...
      {shift ? (
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-1.5">
            <span
              className={cn("size-2.5 rounded-full shrink-0", TONE_DOT[shift.tone])}
              aria-hidden
            />
            <span className="text-body text-text-primary font-medium truncate">{shift.name}</span>
            {shift.isDraft && (
              <span
