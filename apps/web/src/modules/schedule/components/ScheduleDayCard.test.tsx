...
describe("ScheduleDayCard", () => {
  it("shows shift name, time range and weekday/date", () => {
    render(<ScheduleDayCard date="2026-06-08" isToday={false} isWeekend={false} shift={shift} />)
    expect(screen.getByText("Morning")).toBeInTheDocument()
    expect(screen.getByText("09:00–17:00")).toBeInTheDocument()
    expect(screen.getByText(/Mon/)).toBeInTheDocument()
  })
  it("shows Off when no shift", () => {
    render(<ScheduleDayCard date="2026-06-08" isToday={false} isWeekend={false} shift={null} />)
