...
export function validate(f: EmailConfigForm, hasPassword: boolean): Record<string, string> {
  ...
  if (f.enabled) {
    ...
    if (f.use_auth) {
      ...
      if (!f.smtp_password && !hasPassword) e.smtp_password = "Required when authentication is on."
    }
  }
  return e
}
...
export function warnings(f: EmailConfigForm): string[] {
  ...
  if (f.use_auth && !f.smtp_username.trim()) w.push("Authentication is on but no username is set.")
  if (f.encryption === "none") w.push("No encryption — credentials will be sent in clear text.")
  if (f.enabled && !f.smtp_host.trim()) w.push("Email is enabled but no SMTP host is configured.")
  return w
}
