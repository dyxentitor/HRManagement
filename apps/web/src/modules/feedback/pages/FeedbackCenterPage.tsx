...
// Upload limits — must mirror the server allowlist + ceiling in
// FeedbackAttachmentService (image/pdf/video, 25 MB).
const MAX_FILE_BYTES = 25 * 1024 * 1024
const ACCEPT_ATTR =
  "image/png,image/jpeg,image/gif,image/webp,application/pdf,video/mp4,video/quicktime,video/webm"
    ...
    try {
      ...
      for (const f of files) {
        try {
          ...
          failedCount += 1 // upload the rest; don't abandon on first failure
        }
      }
      if (failedCount > 0) {
        toast.success(
          `Feedback submitted, but ${failedCount} of ${files.length} attachment(s) failed to upload.`,
        )
      } else {
  ...
  const columns: Column<FeedbackItem>[] = [
    {
      ...
      header: "Category",
      render: (row) => (
        <span className="text-text-secondary">{CATEGORY_LABELS[row.category] ?? row.category}</span>
      ),
      sortable: true,
    ...
    },
    {
      key: "title",
      header: "Title",
      render: (row) => <span className="text-text-primary font-medium">{row.title}</span>,
      sortable: true,
        ...
        <form onSubmit={onSubmit} className="glass-surface rounded-2xl p-5 space-y-4">
          ...
          <div>
            ...
            >
              ...
              <span className="text-small text-text-tertiary">Max 25 MB · Images, PDF, video</span>
              <input
                id="feedback-file"
                type="file"
                multiple
                ...
                onChange={(e) => {
                  ...
                  if (tooBig.length > 0) {
                    setFormError(`Each file must be under 25 MB (${tooBig[0].name} is too large).`)
                  }
                  // Append (not replace) and skip oversize files.
                  setFiles((prev) => [...prev, ...picked.filter((f) => f.size <= MAX_FILE_BYTES)])
                ...
                {files.map((f, index) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="shrink-0 text-text-tertiary hover:text-coral transition-colors"
                    >
          ...
          <Button type="submit" disabled={!canSubmit} className="w-full soft-glow rounded-xl">
        ...
        <div className="glass-surface rounded-2xl p-5">
          ...
          {loading ? (
            <div className="space-y-2">
              {["a", "b", "c"].map((k) => (
                <div key={k} className="h-10 rounded bg-surface-hover animate-pulse" />
              ))}
            </div>
          ) : (
            <DataTable
        ...
        {selected && (
          <dl className="grid grid-cols-[100px_1fr] gap-y-2 text-body">
            ...
            {selected.attachments && selected.attachments.length > 0 && (
              <>
                <dt className="text-label uppercase text-text-tertiary self-start">Attachments</dt>
                <dd>
