import { ChevronDown, ChevronUp, ChevronsUpDown, Lock, Search } from "lucide-react";
...
export function ProjectsTable({ projects, onChanged }: Props) {
	...
	const sorted = useMemo(() => {
		return [...filtered].sort((a, b) => {
			...
			return sortDir === "asc" ? cmp : sortKey === "deadline" && (!a.deadline || !b.deadline)
					...
					<table className="w-full text-small">
						<thead>
							<tr className="text-label text-text-tertiary">
								...
								<th
									className="text-left font-medium pb-2"
									aria-sort={sortKey === "deadline" ? (sortDir === "asc" ? "ascending" : "descending") : "none"}
								>
									<button
										type="button"
										className={thBtn}
											...
											<td className="py-2.5">
												<div className="flex items-center gap-2">
													<button
														...
														className="text-accent-300 hover:text-accent-200 text-[11px] transition-colors disabled:opacity-40"
													>
														Edit
													</button>
													{p.status === "open" ? (
														<button
															...
															className="text-coral hover:text-coral/80 text-[11px] transition-colors"
														>
															Close
														</button>
													) : (
														<button
															...
															className="text-mint hover:text-mint/80 text-[11px] transition-colors"
														>
															Reopen
														</button>
													)}
