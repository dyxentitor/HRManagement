	...
	return (
		<div className="glass-surface rounded-2xl p-4">
							...
							{pageRows.map((c) => (
								<tr key={c.id} className="border-t border-border-subtle">
									...
									<td className="py-2.5 text-text-secondary">
										{Number(c.mandays_remaining).toLocaleString("en-MY", { maximumFractionDigits: 0 })}
										{" / "}
										...
										<td className="py-2.5">
											<div className="flex items-center gap-2">
												<button
													...
													className="text-accent-300 hover:text-accent-200 text-[11px] transition-colors"
												>
													Edit
												</button>
												<button
													...
													className="text-sky hover:text-sky/80 text-[11px] transition-colors"
												>
													Top up
												</button>
												<button
													...
													className={
														c.is_active
															? "text-coral hover:text-coral/80 text-[11px] transition-colors"
															: "text-mint hover:text-mint/80 text-[11px] transition-colors"
													}
												>
			...
			<EditCustomerModal
				...
				onDone={() => { void fetchRows(includeInactive); onChanged(); }}
				customer={editTarget}
			/>
			...
			<TopUpModal
				...
				pools={
					topUpTarget
						? [{ id: topUpTarget.id, name: topUpTarget.name, remaining: topUpTarget.mandays_remaining, total: topUpTarget.mandays_total, project_count: 0, pct_used: 0 }]
						: []
				}
			/>
