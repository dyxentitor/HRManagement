...
| `v1.11.0` | 2026-06-08 | Unified user/employee creation. **Progressive employee create** — only 7 mandatory fields (`employee_code`, `first_name`, `last_name`, `email`, `hire_date`, `department`, `employment_type`); rest nullable (migration `employee.0004`) + "Complete later" form sections. **Employee-first provisioning** — optional `provision` block on `POST /employees/` creates+links a `User` atomically. **User-first page** `/admin/settings/users/new` + `POST /api/v1/users/` (`UserCreateView`). Shared `provision_user` service (invite reuses v1.2.0 reset flow / temp password). **Forced password change** — `User.must_change_password` (migration `identity.0005`) surfaced in login+`/me`, enforced by `SignedOutGate` → `/force-password-change` → `POST /auth/password/change`. **`profile_completeness`** field + employee-detail banner. New perm **`user:create`** (org_admin + hr_manager; 110→111; backfill via `grant_default_perms`). | `Prompt_v1.11.0_unified_user_employee_creation.md` | `2026-06-08-v1.11.0-unified-user-employee-creation.md` |

### 2.3 Test counts at HEAD (v1.11.0)

- Backend: **739 passed** (+38 vs v1.10.1). One pre-existing, unrelated failure carried forward: `modules/attendance/tests/test_clock_flow.py::test_clock_out_completes_record` (date-sensitive; fails identically on `master`, v1.11.0 touches no attendance code — fix separately).
- Frontend: **298 passed** (+20 vs v1.10.1).
- Permission codes: **111** (+1: `user:create`).
